...
template <typename Key>
std::pair<typename RBTree<Key>::iterator, bool> RBTree<Key>::InsertRecursive(
    Node<Key> *parent, Node<Key> *child) {
  Node<Key> **tmp =
      (Less(*(child->key), *(parent->key))) ? &parent->left : &parent->right;

  std::pair<iterator, bool> result{End(), false};

  if (!allow_equal_ && Equal(*(parent->key), *(child->key))) {
    result =
        std::pair<iterator, bool>{RBTree<Key>::iterator(parent, this), false};
  } else if (*tmp == nullptr) {
    *tmp = child;
    child->parent = parent;
    InsCase1(child);
    size_++;
    result =
        std::pair<iterator, bool>{RBTree<Key>::iterator(child, this), true};
  } else {
...
template <typename Key>
std::pair<typename RBTree<Key>::iterator, bool> RBTree<Key>::Insert(
    const Key key) {
  Node<Key> *new_node =
      new Node<Key>{new Key{key}, nullptr, nullptr, nullptr, red_node};
  std::pair<iterator, bool> result{End(), false};
