...
class RBTree<Key>::Iterator {
 ...
 public:
  ...
  Iterator operator++() {
    if (!node_) return Iterator(nullptr, parent_);
  ...
  bool operator==(const Iterator &other) const {
    return (node_ == other.node_)     ? true
           : (!node_ || !other.node_) ? false
...
class RBTree<Key>::ConstIterator : public Iterator {
 ...
 public:
  ...
  const_reference operator*() const { return *(node_->key); }
};
...
typename RBTree<Key>::ConstIterator RBTree<Key>::cBegin() const {
  return typename RBTree<Key>::ConstIterator(GetMin(root),
                                             const_cast<RBTree<Key> *>(this));
}

template <typename Key>
typename RBTree<Key>::ConstIterator RBTree<Key>::cEnd() const {
  return typename RBTree<Key>::ConstIterator(nullptr,
                                             const_cast<RBTree<Key> *>(this));
}
}  // namespace s21
