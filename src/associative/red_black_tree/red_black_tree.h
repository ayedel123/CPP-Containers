...
struct Node {
  ...
  ~Node() { delete key; }
  bool operator<(const Node<Key> &other) const { return *key < *other.key; }
};
...
class RBTree {
 ...
 public:
  RBTree()
      : Less([](const Key &v1, const Key &v2) { return v1 < v2; }),
        Equal([](const Key &v1, const Key &v2) { return v1 == v2; }) {}
  RBTree(bool (*lessFn)(const Key &, const Key &),
         bool (*equalFn)(const Key &, const Key &))
  ...
  void SetComparisonFunctions(bool (*lessFn)(const Key &v1, const Key &v2),
                              bool (*equalFn)(const Key &v1, const Key &v2)) {
    Less = lessFn;
    Equal = equalFn;
  }
  ...
  // Insert
  std::pair<RBTree<Key>::iterator, bool> InsertRecursive(Node<Key> *parent,
                                                         Node<Key> *child);
      ...
      while (nodeCount > 0) {
        ...
        if (node) {
          if (node->color == 1) {
            ...
            std::cout << "\033[1;31m";  // ASCII-код для установки цвета текста
                                        // в красный
          } else {
            // Устанавливаем черный цвет для текста
            std::cout << "\033[1;30m";  // ASCII-код для установки цвета текста
                                        // в черный
          }
