...
void RBTree<Key>::DelCase3(Node<Key> *node) {
  ...
  if (parent->color == black_node && sibling_color == black_node &&
      lcolor == black_node && rcolor == black_node) {
    if (sibling) {
...
void RBTree<Key>::DelCase4(Node<Key> *node) {
  ...
  if (parent->color == red_node && sibling_color == black_node &&
      lcolor == black_node && rcolor == black_node) {
    if (sibling) {
      sibling->color = red_node;
    }
...
void RBTree<Key>::DelCase5(Node<Key> *node) {
  ...
  if (sibling->color == black_node) {
    if (node == parent->left && rcolor == black_node && lcolor == red_node) {
      ...
      RotateRight(sibling);
    } else if (node == parent->right && lcolor == black_node &&
               rcolor == red_node) {
      sibling->color = red_node;
